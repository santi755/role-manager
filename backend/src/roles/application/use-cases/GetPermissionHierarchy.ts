...
export class GetPermissionHierarchy {
    ...
    async execute(query: GetPermissionHierarchyQuery): Promise<PermissionHierarchyDto> {
        ...
        const ancestorIds = this.permissionGraphService.resolvePermissionDependencies(
            permission,
            permissionsMap,
        );
        ...
        for (const [, potentialChild] of permissionsMap) {
            const childAncestors = this.permissionGraphService.resolvePermissionDependencies(
                potentialChild,
                permissionsMap,
            );
            if (
                ...
                descendants.push({
                    ...
                    parentPermissions: Array.from(potentialChild.getParentPermissions()).map(id => id.toString()),
                });
            }
        }
        ...
        return {
            ...
            ancestors: ancestors.map((ancestor) => ({
                ...
                parentPermissions: Array.from(ancestor.getParentPermissions()).map((id) =>
                    id.toString(),
                ),
            })),
