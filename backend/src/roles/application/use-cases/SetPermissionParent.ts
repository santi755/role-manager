...
export class SetPermissionParent {
  ...
  async execute(command: SetPermissionParentCommand): Promise<void> {
    ...
    const parentPermissionId = PermissionId.fromString(
    ...
    const parentPermission =
      await this.permissionRepository.findById(parentPermissionId);
    if (!parentPermission) {
      throw new Error(
        `Parent permission with id ${command.parentPermissionId} not found`,
      );
    }
    ...
    if (
      this.permissionGraphService.wouldCreateCycle(
        permission,
        parentPermissionId,
        permissionsMap,
      )
