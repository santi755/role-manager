...
export class MongoPermissionRepository implements PermissionRepository {
    ...
    async save(permission: Permission): Promise<void> {
        const doc = {
            ...
            parentPermissions: Array.from(permission.getParentPermissions()).map((p) =>
                p.toString(),
            ),
        };
    ...
    async findById(id: PermissionId): Promise<Permission | null> {
        const doc = await this.permissionModel.findOne({ id: id.toString() }).exec();
        if (!doc) return null;
    ...
    async findByIds(ids: PermissionId[]): Promise<Permission[]> {
        ...
        const docs = await this.permissionModel.find({ id: { $in: idStrings } }).exec();
        return docs.map((doc) => this.toDomain(doc));
    }
