...
export class MongoUserRepository implements UserRepository {
  ...
  async save(user: User): Promise<void> {
    ...
    await this.userModel.findOneAndUpdate({ id: doc.id }, doc, {
      upsert: true,
      new: true,
    });
  }
  ...
  private toDomain(doc: UserDocument): User {
    return User.reconstitute(
      ...
      new Set(
        doc.directPermissionGrants.map((p) => PermissionId.fromString(p)),
      ),
      new Set(
        doc.directPermissionDenials.map((p) => PermissionId.fromString(p)),
      ),
    );
  }
}
